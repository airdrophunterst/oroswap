...
function loadSeedPhrases() {
  try {
    ...
    seedPhrases = data
      ...
      .filter((phrase) => phrase.split(" ").length >= 12);
...
function updateStatus() {
  ...
  const status = swapRunning
    ...
    : chalk.green("Idle");
  const statusText = `Status: ${status} | Active Account: ${getShortAddress(walletInfo.address)} | Total Accounts: ${seedPhrases.length} | Auto Swap: ${
